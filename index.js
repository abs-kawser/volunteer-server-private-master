...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.n8dqd.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });


client.connect(err => {
    const collection = client.db("db_volunteer_net").collection("coll_event_task") // for event list
    const collectionUserEvent = client.db("db_volunteer_net").collection("coll_user") // for users event
    const collectionAdminAuth = client.db("db_volunteer_net").collection("coll_admin_auth") // for admin auth
...
//skisshdjhb
    ...
    app.get('/userEventList/:email', (req,res)=>{ // events only
